...
mod tests {
    ...
    fn embedded_migrator_contains_consolidated_initial_schema() {
        ...
        // FORK-LOCAL PATCH (adrienlacombe/buzz): upstream ships 32 migrations; this
        // fork adds two of its own, so the count is 34 here.
        //
        ...
        // `0029_channels_id_lookup_index.sql` here, its
        ...
        // `0032_community_deletion_recovery.sql`, its
        // `0031_workflow_run_error_codes.sql` is
        // `0033_workflow_run_error_codes.sql`, and its
        // `0032_channel_roster_snapshot_fence.sql` is
        // `0034_channel_roster_snapshot_fence.sql`. See the assertions for each
        // below.
        ...
        // (upstream's 0029, FORK-LOCAL PATCH (adrienlacombe/buzz): renumbered to
        // 0031 here because this fork already holds 0027 and 0028).
        ...
        // Recovery migration alters populated tables and must preserve the same
        // fail-fast lock behavior as the deletion migration (upstream's 0030,
        // FORK-LOCAL PATCH (adrienlacombe/buzz): renumbered to 0032 here because
        // this fork already holds 0027 and 0028).
        ...
        // (upstream's 0032, FORK-LOCAL PATCH (adrienlacombe/buzz): renumbered to
        // 0034 here because this fork already holds 0027 and 0028. Upstream wrote
        // this assertion against index 31 / version 32, which in this tree is the
        // fork's own `0032_community_deletion_recovery.sql` — it merges as clean
        // context, so the index and the version both shift by the fork's two.)
        assert_eq!(migrations[33].version, 34);
        let roster_fence = migrations[33].sql.as_str();
