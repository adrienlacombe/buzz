...
mod tests {
    ...
    async fn unmigrated_roster_fence_blocks_startup_until_0032_is_applied() {
        ...
        // FORK-LOCAL PATCH (adrienlacombe/buzz): upstream migrates through 31 here,
        // the version immediately below its own `0032_channel_roster_snapshot_fence`.
        // The fork holds 0027 and 0028, so that fence is `0034` here and the version
        // immediately below it is 33. Only the target literal moves; the test name
        // and messages are left as upstream wrote them so this stays one hunk.
        let (pool, scratch_name) =
            create_scratch_db_through(&admin, "roster_fence_unmigrated", Some(33)).await;
