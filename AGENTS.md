...
It has now happened on four separate syncs, covering five migrations —
...
`0031_workflow_run_error_codes.sql` (upstream #5780) in the 2026-08-14 sync;
renumbered to `0029`, `0030`, `0031`, `0032` and `0033`. It has now landed on four
consecutive syncs that touched `migrations/`, which is every one of them — treat a
collision as the default outcome, not a possibility. Note that the *second*
