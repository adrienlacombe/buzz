...
> **All three sync workflows are currently `disabled_manually`.** A local
> scheduled task runs the sync from a developer machine instead, and it is the
> only writer today. Do not re-enable them without retiring that task — two
> writers racing on the same sync branch is worse than one. Everything below
> describes the GitHub-hosted path, kept as the fallback and as the record of why
> it is shaped the way it is; the *reviewing a sync PR* guidance at the end
> applies either way, because it is about the merge, not about what produced it.
...
| `desktop/src/features/messages/lib/messageLink.test.mjs` | asserts against a `SCHEME` const instead of a repeated `buzz://` literal | Companion to the `messageLink.ts` row below. The legacy `buzz://` literals further down the file are deliberate and must **not** follow the rename — those tests exist to prove links already in message history still parse |
| `.github/aw/actions-lock.json` | new | gh-aw action SHA pins |
| `.gitattributes` | `.github/workflows/*.lock.yml linguist-generated=true merge=ours` | Added by `gh aw init`. **`merge=ours` is inert here**: it names a merge driver that is not defined, and git silently falls back to a normal merge when a driver is missing. Either configure it (`git config merge.ours.driver true`, per clone — it cannot be committed) or read this row as documentation-only. Do not rely on it protecting the lock file during a merge |
| `ci.yml` | mesh-llm rev read from `desktop/src-tauri/Cargo.lock` | The two locks pin mesh-llm independently (desktop is outside the root workspace) and can name different revs — at the time of the patch, root `tag=v0.73.1` (`43103c5c`) vs desktop `rev=f455d493`. The step fetches the *desktop* manifest, so the root rev names a checkout never fetched. Upstream is masked by a warm cache — the step is skipped on cache hit. **Both locks currently pin the same rev — `tag=v0.75.1` (`3295c902`) as of the 2026-08-16 sync — so the patch is a temporary no-op. Do not delete it.** The locks stay independent; the next bump that moves one and not the other re-breaks the root-lock version. The revs converging is a coincidence that has now held across several bumps (`v0.74.0`/`e60b2fe4`, then `v0.75.1`), not upstream adopting the fix — re-derive both with `python3 -c 'import tomllib; …'` rather than trusting this row's version |
...
to upstream** and is listed here so it stays that way. Fork-side dependabot (PR #18)
bumped two packages in it and, having no access to Block's internal Artifactory,
re-resolved the whole file against public PyPI — rewriting all 1645 index URLs and
diverging 1672 lines. It was reverted in the 2026-08-16 audit.

Three things made it worth reverting rather than keeping, and they generalise:

- **Nothing reads it.** `benchmark-harbor.yml` installs with
  `pip install -e ".[dev]"`, never `uv sync`, so the lock is inert in CI. The
  dependency bumps it carried were therefore not a real security posture change.
- **It is upstream's file, and upstream rewrites it.** 1672 lines of divergence in
  a file this fork does not consume is the most expensive kind of conflict surface
  there is.
- **It arrived unmarked.** A dependabot PR carries no `FORK-LOCAL` comment and no
  table row, so it reads as upkeep rather than as a fork decision.

**A dependabot PR touching a lock file that pins Block's Artifactory is the
tripwire** — check whether the fork actually consumes the lock before merging. If
one does need to diverge someday, that is a decision to record in the table above,
not something to inherit from a bot.
...
It has now happened on five separate syncs, covering six migrations —
...
(upstream #4425) together in the 2026-08-13 sync, then
`0031_workflow_run_error_codes.sql` (upstream #5780) in the 2026-08-14 sync, then
`0032_channel_roster_snapshot_fence.sql` (upstream #6251) in the 2026-08-22 sync;
renumbered to `0029`, `0030`, `0031`, `0032`, `0033` and `0034`. It has now landed on
five consecutive syncs that touched `migrations/`, which is every one of them — treat
a collision as the default outcome, not a possibility. Note that the *second*
collision is the more dangerous shape: upstream's 0028 landed on the fork's 0028,
so the two files sorted adjacent and the tree looked plausible. The 2026-08-13 sync
was that shape twice over — both of upstream's new files landed on fork-held
integers — and the 2026-08-14 sync was the same shape again, upstream's 0031
landing on the fork's `0031_community_deletion.sql`. **A new file under
`migrations/` is the tripwire — check the version integer before reading anything
else in the diff.**
...
| 2026-08-14 | 0031 → 0033 | `migrations[30].version == 31` → `migrations[32].version == 33`, plus the `migrations[30].sql` binding on the next line, in `workflow_run_error_codes_are_additive_and_backfilled_without_parsing_diagnostics`; `migrations.len()` 31 → 33. That test's other lookup, `find(\|m\| m.version == 1)`, is the initial schema and does **not** move — a version literal is not automatically a renumber site, so read what it resolves to before shifting it |
| 2026-08-22 | 0032 → 0034 | `migrations[31].version == 32` → `migrations[33].version == 34`, plus the `migrations[31].sql` binding on the next line, in `embedded_migrator_contains_consolidated_initial_schema`; `migrations.len()` 32 → 34. **This one collided with an existing assertion rather than merely being wrong**: the fork's own deletion-recovery assertion already claims `migrations[31]`, so after the merge two assertions in the same test both read index 31 and the roster-fence block silently pointed at `0032_community_deletion_recovery.sql`. Two identical `assert_eq!(migrations[31].version, 32)` lines a few lines apart is the tell. Plus a **fourth shape in a different file** — see the `crates/buzz-db/src/lib.rs` row above |
...
**And there is now a fourth shape, outside this file entirely.** The 2026-08-22 sync
added a *bounded-migration target*: `crates/buzz-db/src/lib.rs` calls
`create_scratch_db_through(…, Some(31))` to build a database stopped just below
upstream's roster fence. That is neither a count, an index, nor a `version == N`
lookup, so all three sweeps above miss it, and the test carrying it is
`#[ignore = "requires Postgres"]` so no local gate objects. Add
`create_scratch_db_through` and `run_to(` to the grep list, and search the whole
`crates/buzz-db/` tree rather than just `migration.rs`.
...
`scripts/sign-binding.html` — a browser page that signed the SNIP-12 binding
document with an injected Starknet wallet — outlived that removal by several
weeks, orphaned and referenced by nothing, and was deleted in the 2026-08-16
audit. What survives of `wallet` is the *account* surface: `deploy`, `class-hash`
and `address`, all serving `contracts/src/account.cairo`. A file whose only
inbound reference is the removed feature is the shape to sweep for next time —
`grep -ril nip-sw` over the tree is the cheap check, and the two `migrations/0027`
/`0028` files are its only legitimate remaining hits besides the prose here.
...
| `Auto-tag on Release PR Merge` | **should be disabled — currently `active`** | Fires on every merged same-repo PR. A sync PR carrying a `deploy/charts/buzz/Chart.yaml` version bump hits its default lane and tries to mint a token from the `BUZZ_RELEASE_TAGGER` GitHub App, which does not exist here. Audited `active` on 2026-08-16 despite this row; either it was re-enabled or the disable never took. Harmless while sync PRs are not being merged, but re-disable before the next one: `gh workflow disable "Auto-tag on Release PR Merge" --repo adrienlacombe/buzz` |
...
| All three sync workflows | **`disabled_manually` on purpose** | `Sync this fork with upstream`, `Upstream sync (merge)` and `Upstream sync CI status` are all off, superseded by a local scheduled task that does the sync from a developer machine. **Do not re-enable them** — two writers racing on the same sync branch is worse than one. The workflow files are kept as the fallback path, and the [How the sync works](#how-the-sync-works) section still describes what they do if that decision is reversed. Their state is visible only via `gh workflow list --all --repo adrienlacombe/buzz`; a disabled sync workflow never goes red |
