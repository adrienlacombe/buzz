...
/// Build the global-shortcut plugin that drives PTT press/release.
///
/// Omitted from test builds: linking it into the lib-test binary makes it fail
/// to load on Windows (`STATUS_ENTRYPOINT_NOT_FOUND`) before any test runs.
#[cfg(not(test))]
pub fn global_shortcut_plugin<R: tauri::Runtime>() -> tauri::plugin::TauriPlugin<R> {
    ...
    // Generation counter for the release delay task. Incremented on every
    // press — a delayed release only fires if the generation hasn't changed
    // (i.e. no new press happened during the delay). This prevents
    // press→release→press within 200 ms from having the first release clobber
    // the second press.
            ...
            match event.state {
                ShortcutState::Pressed => {
                    ...
                    if let Ok(hs) = state.huddle_state.lock() {
                        ...
                        // Only cancel TTS if it's actually playing — avoids a
                        // stale cancel flag that drops the next queued message.
                        if hs.tts_active.load(std::sync::atomic::Ordering::Acquire) {
                            hs.tts_cancel
                                .store(true, std::sync::atomic::Ordering::Release);
                        }
                    }
                    // Emit ptt-state=true to the frontend. The React side plays
                    // the press audio cue on this event (Web Audio API via
                    // HuddleContext). Rust-side rodio audio was considered but
                    // rejected: the rodio OutputStream must outlive the handler
                    // and sharing it across the shortcut closure adds lifecycle
                    // complexity for marginal gain.
                    let _ = app.emit("ptt-state", true);
                }
                ShortcutState::Released => {
                    let gen_at_release = ptt_press_gen.load(std::sync::atomic::Ordering::Acquire);
